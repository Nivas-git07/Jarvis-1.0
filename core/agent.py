...
from tools.reminder import  save_reminder,get_reminders
import json
...
while True:
    ...
    if any(word in task_lower for word in reminder_keywords):

        reminder_completion = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
               {
    "role": "system",
    "content": """
You extract reminders.
...
"""
        ...
        try:
            ...
            print(
                f"\nFRIDAY: Got it boss. I've saved a reminder for {data['title']} at {data['datetime']}.\n"
            )

        except Exception as e:

            print(
                f"\nFRIDAY: I couldn't save that reminder. {e}\n"
            )
